...
You are a Senior Enterprise QA Engineer writing test cases for a formal test plan.
...
Write exactly ONE high-quality test case for this specific scenario:
...
Quality rules — every field must be specific, not generic:

title: Write a precise title that names the exact field, action, or rule being tested.
       Use actual domain terms from the requirement (e.g. field names, action names, system names).
       BAD: "User submits the form"
       GOOD: "iBuy delivery address Create request fails when Name field is left empty"

preconditions: Describe the exact state of the system BEFORE the test begins.
       Must be consistent with the steps — if the steps enter invalid data, preconditions must NOT say "valid data is entered".
       BAD: "User is logged in"
       GOOD: "User is logged into the ServiceNow portal, has navigated to the Manage iBuy Delivery Address form, and has selected Create as the action with all other fields completed except Name"

test_data: List the specific data values used in this test — field names and values.
       Never leave empty. Never write "valid data" without specifying what that data is.
       BAD: "Valid form data"
       GOOD: "Action: Create, Title: Mr., Name: (left empty), House number: 10, Street: King Street, Postal code: 2000, City/Suburb: Sydney"

steps: Write 4-7 clear, numbered steps that a human tester can follow without guessing.
       Each step must be an action verb. Reference actual field names from the requirement.
       BAD: ["Fill in the form", "Submit"]
       GOOD: ["Navigate to the Manage iBuy Delivery Address form", "Select Create from the What would you like to do? dropdown", "Complete all mandatory fields except Name", "Leave the Name field empty", "Click Submit"]

expected_result: State one definite, specific outcome — what the tester will see or verify.
       Never write a conditional outcome ("displays an error if...").
       BAD: "An error is shown if the field is empty"
       GOOD: "The system displays a mandatory field validation error for Name and the request is not submitted"

automation_candidate: "Yes" if the test can be automated with a tool like Tosca or Selenium. "No" if it requires human judgment or manual environment setup.

source_traceability: Name the requirement document or section this scenario comes from.
...
{{
  "test_cases": [{{
    ...
    "title": "<specific title using actual domain terms>",
    "priority": "{priority}",
    "test_type": "{test_type}",
    "preconditions": "<specific system state before test — consistent with steps>",
    "test_data": "<specific field names and values used in this test>",
    "steps": ["<action step 1 using real field names>", "<action step 2>", "<action step 3>", "<action step 4>"],
    "expected_result": "<single definite outcome — what the tester sees or verifies>",
...
def generate_test_case_for_scenario(
    ...
    Includes automatic retry with backoff for Groq rate limit errors (429).
    """
    ...
    if raw_text is None:
        ...
        # Small delay between calls to stay within Groq rate limits
        # even when not hitting a 429 — prevents burst requests from
        # triggering rate limits on subsequent calls.
        time.sleep(1.5)
