...
def _build_retry_prompt(group_file, requirement, analysis, rag_context, previous_count):
    """
    A more forceful version of the group prompt used when the first
    attempt returned too few test cases (0 or 1). Instead of the normal
    "MINIMUM EXPECTATION" soft instruction, this explicitly tells the
    model how many it returned last time and demands more.
    """

    base_prompt = _build_prompt(group_file, requirement, analysis, rag_context)

    retry_prefix = (
        f"IMPORTANT: Your previous attempt for this coverage group only "
        f"returned {previous_count} test case(s). That is not enough. "
        f"You MUST generate at least 2 test cases for this group. "
        f"Read the requirement again carefully and identify EVERY distinct "
        f"behavior, scenario, or workflow it describes for your assigned "
        f"coverage areas. Generate one test case per distinct behavior. "
        f"Do not stop after one test case.\n\n"
    )

    return retry_prefix + base_prompt
...
def _generate_single_group(
    ...
    """
    Runs ONE coverage-group generation call, with automatic retry if the
    first attempt returns fewer than min_test_cases usable test cases.

    Retry uses a more forceful prompt that explicitly tells the model
    how many it returned last time and demands more - this is more
    effective than just re-running the same prompt, which would likely
    produce the same thin result.

    If stream_queue is provided, pushes ("chunk", group_id, accumulated_text)
    messages to it as tokens arrive so the UI can show live progress.
    """
    ...
    def _invoke_llm(prompt):
        ...
        try:
            ...
            if stream_queue is not None:
                stream_queue.put(("chunk", group_id, raw))
            return raw

    def _parse_and_filter(raw_text):
        was_trunc = False
        err = None
        try:
            parsed, was_trunc = parse_json_safely(raw_text, llm)
        ...
        tcs, dropped_ot = _filter_to_allowed_areas(tcs, allowed_areas, group_id)

        if use_copy_filter:
            tcs, dropped_ce = _filter_copied_examples(tcs, group_file)
        else:
            dropped_ce = 0

        return tcs, was_trunc, dropped_ot, dropped_ce, err

    # ── First attempt ────────────────────────────────────────────────────────

    prompt = _build_prompt(group_file, requirement, analysis, rag_context)
    cache_key = make_cache_key("generation", model_name, group_id, prompt)
    ...
    test_cases, was_truncated, dropped_off_topic, dropped_copied_example, error = (
        _parse_and_filter(raw_text)
    )

    # ── Retry if too few test cases ──────────────────────────────────────────

    retry_count = 0

    while len(test_cases) < min_test_cases and retry_count < max_retries and not from_cache:

        retry_count += 1
        retry_prompt = _build_retry_prompt(
            group_file, requirement, analysis, rag_context, len(test_cases)
        )

        if stream_queue is not None:
            stream_queue.put(("chunk", group_id, f"[Retry {retry_count}: previous attempt returned only {len(test_cases)} test case(s). Retrying...]\n\n"))

        raw_text = _invoke_llm(retry_prompt)

        retry_tcs, retry_trunc, retry_ot, retry_ce, retry_err = _parse_and_filter(raw_text)

        if len(retry_tcs) > len(test_cases):
            test_cases = retry_tcs
            was_truncated = retry_trunc
            dropped_off_topic += retry_ot
            dropped_copied_example += retry_ce
            if retry_err:
                error = retry_err
