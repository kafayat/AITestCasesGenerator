...
from agents.scenario_generator_agent import (
    generate_all_scenarios_concurrent,
)
...
st.title("Enterprise AI Test Case Generator - Phase 16")
st.caption(
    "Phase 16 adds Groq (free, fast), Azure AI Foundry, and Ollama as provider options — "
    "select Groq in the sidebar for 28 test cases in under 60 seconds."
)
...
provider = st.sidebar.selectbox(
    "Model Provider",
    ["Groq (free, fast)", "Azure AI Foundry", "Ollama (local)"],
    index=0,
)
...
if provider == "Groq (free, fast)":
    ...
    st.sidebar.caption(
        ...
        "Free tier: 6,000 tokens/min for 70B. Switch to 8b-instant if rate limits hit."
    )
    ...
    generation_model = st.sidebar.selectbox(
        ...
        groq_models,
        index=0,
        help="llama-3.3-70b-versatile recommended — best quality. Uses more tokens/min so runs slightly slower than 8B.",
    )

    quality_model = generation_model
    provider = "Groq"
    ...
    generation_model = azure_deployment
    quality_model = azure_deployment

    st.sidebar.caption(
        "GPT-4o-mini recommended — ~$0.002 per 28-scenario run. "
        "New Azure accounts get $200 free credit."
    )

    if not azure_endpoint or not azure_api_key:
        st.sidebar.warning("Enter your Azure endpoint and API key to generate.")
    ...
    st.sidebar.caption(
        "Ollama runs locally — no internet needed but slower (~90s per scenario). "
        "Run with OLLAMA_NUM_PARALLEL=2 for best results."
    )
...
# Parallel workers only relevant for Ollama (Groq/Azure handle concurrency server-side)
if provider == "Ollama":
    max_parallel_workers = st.sidebar.slider(
        "Parallel scenario calls (match OLLAMA_NUM_PARALLEL)",
    ...
    max_parallel_workers = 2  # 70B on free tier: 2 concurrent + 1.5s delay keeps within 6k TPM
    max_scenarios = 0
...
tab_upload, tab_requirement, tab_generate, tab_coverage, tab_quality, tab_history = st.tabs([
...
with tab_generate:
    ...
    with generate_col:
        provider_ready = (
            ...
            (provider == "Azure AI Foundry" and azure_endpoint and azure_api_key) or
            (provider == "Ollama")
        )
    ...
    with clear_col:
        if st.button("Clear Knowledge Base", help="Remove all uploaded files and reset before a new project"):
            clear_knowledge_base("knowledge_base", "vector_store")
            for key in ("detected_requirement", "requirement_source", "phase14_results"):
                st.session_state.pop(key, None)
            st.success("Knowledge base cleared. Upload your new files in Tab 1.")

    if generate_clicked:

        start_time = time.time()

        generation_llm = get_llm(provider, generation_model, task="generation", groq_api_key=groq_api_key, azure_endpoint=azure_endpoint, azure_api_key=azure_api_key)
        analysis_llm = get_llm(provider, generation_model, task="analysis", groq_api_key=groq_api_key, azure_endpoint=azure_endpoint, azure_api_key=azure_api_key)
        inference_llm = get_llm(provider, generation_model, task="inference", groq_api_key=groq_api_key, azure_endpoint=azure_endpoint, azure_api_key=azure_api_key)
        # Use a smaller/faster model for quality scoring to preserve
        # the daily token budget for the actual test case generation.
        # Quality scoring is a single call so 8B is sufficient.
        quality_scoring_model = (
            "llama-3.1-8b-instant"
            if provider == "Groq" and "70b" in generation_model
            else quality_model
        )
        quality_llm = get_llm(provider, quality_scoring_model, task="scoring", groq_api_key=groq_api_key, azure_endpoint=azure_endpoint, azure_api_key=azure_api_key)
        review_llm = get_llm(provider, quality_model, task="review", groq_api_key=groq_api_key, azure_endpoint=azure_endpoint, azure_api_key=azure_api_key)
        ...
        # Stale knowledge base detection: if the retrieved RAG context
        # contains source files whose names don't appear to relate to the
        # current requirement, warn the user before generation starts.
        # The most common cause is a previous run's sample test cases or
        # knowledge files still sitting in the knowledge_base folder,
        # which the model then anchors to instead of the real requirement.
        ...
        if retrieved_sources and requirement.strip():
            req_words = set(re.findall(r"[a-z0-9]+", requirement.lower()[:500]))
            stale_sources = []
            for source in retrieved_sources:
                source_words = set(re.findall(r"[a-z0-9]+", source.lower()))
                overlap = req_words & source_words
                meaningful_overlap = {
                    w for w in overlap
                    if w not in {"the", "a", "an", "of", "for", "and", "or", "to", "in", "is", "it", "txt"}
                    and len(w) > 2
                }
            ...
            if stale_sources:
                st.warning(
                    f"**Possible stale knowledge base detected.** "
                    f"The following retrieved source file(s) do not appear to relate "
                    f"to your current requirement: {', '.join(stale_sources)}. "
                    f"This can cause the model to generate test cases from a previous "
                    f"project instead of your current one. "
                    f"Go to Tab 1 and click **Clear Knowledge Base**, then re-upload "
                    f"only your current project files before generating."
                )
        ...
        # ── Phase 15: Scenario-driven generation ──────────────────────────────
        if use_scenario_mode:

            st.subheader("Phase 16 - Scenario Extraction & Generation")
            st.caption(
                "The number of test cases equals the number of testable scenarios "
                "extracted from your requirement — it changes with every requirement document."
            )

            # Step 1: Show extracted scenarios before generation starts
            with st.spinner("Extracting testable scenarios from requirement..."):
                scenarios = extract_scenarios(requirement)
                scenario_summary = summarise_scenarios(scenarios)

            # Apply max_scenarios limit if set
            if max_scenarios > 0 and len(scenarios) > max_scenarios:
                scenarios = scenarios[:max_scenarios]
                st.info(f"Limited to {max_scenarios} scenarios as set in sidebar. Change to 0 for all.")

            st.success(f"Extracted {len(scenarios)} testable scenarios from the requirement.")

            with st.expander(f"View all {len(scenarios)} extracted scenarios"):
                for i, s in enumerate(scenarios, 1):
                    st.write(f"**{i}.** [{s['coverage_area']}] {s['title']}")

            # Step 2: Generate one test case per scenario concurrently
            st.subheader(f"Generating {len(scenarios)} test cases (one per scenario)...")

            progress_bar = st.progress(0, text="Starting...")
            status_placeholder = st.empty()

            # Use a queue for thread-safe progress updates — Streamlit
            # does not allow UI calls from background threads directly,
            # so the callback puts events into a queue and the main
            # thread polls and updates the UI.
            progress_queue = queue.Queue()
            generation_outcome = {}

            def scenario_progress(title, status):
                progress_queue.put((title, status))

            def _run_scenario_generation():
                try:
                    test_cases, scenario_results, summary = generate_all_scenarios_concurrent(
                        generation_llm,
                        requirement,
                        rag_context[:2000],
                    ...
                    generation_outcome["result"] = (test_cases, scenario_results, summary)
                except Exception as exc:
                    generation_outcome["error"] = str(exc)
                finally:
                    progress_queue.put(("__done__", "__done__"))

            gen_thread = threading.Thread(target=_run_scenario_generation, daemon=True)
            gen_thread.start()

            # Poll the queue from the main thread and update Streamlit UI
            completed_count = 0
            failed_count = 0

            while True:
                try:
                    title, status = progress_queue.get(timeout=0.5)
                except Exception:
                ...
                if status in ("done", "cached"):
                    ...
                    failed_count += 1
                    status_placeholder.warning(f"Failed: {title}")

                total_done = completed_count + failed_count
                fraction = min(total_done / len(scenarios), 1.0) if scenarios else 1.0
                elapsed = round(time.time() - generation_start_time, 1)
                progress_bar.progress(
                    fraction,
                    text=f"{total_done}/{len(scenarios)} scenarios complete ({elapsed}s elapsed)"
                )

            gen_thread.join()

            if "error" in generation_outcome:
                st.error(f"Generation failed: {generation_outcome['error']}")
                st.stop()

            test_cases_list, scenario_results, summary = generation_outcome["result"]

            progress_bar.progress(1.0, text="Generation complete.")

            failed_scenarios = [r for r in scenario_results if r["error"]]
            cached_count = sum(1 for r in scenario_results if r["from_cache"])

            elapsed = round(time.time() - generation_start_time, 2)
            st.write(
                f"Generated **{len(test_cases_list)} test cases** from "
                f"{len(scenarios)} scenarios in {elapsed}s "
                f"({cached_count} from cache)."
            )

            if failed_scenarios:
                with st.expander(f"{len(failed_scenarios)} scenario(s) failed to generate"):
                    for r in failed_scenarios:
                        st.write(f"- **{r['scenario']['title']}**: {r['error']}")

            # Wrap into the format the rest of the pipeline expects
            merged_result = {"test_cases": test_cases_list}
            group_results = []
            duplicate_ids = []

        # ── Phase 14: Coverage-group generation (legacy) ──────────────────────
        else:

            st.subheader("Test Case Generation Progress")

            stream_queue = queue.Queue()
            generation_outcome_p14 = {}
            ...
            def _run_generation():
                try:
                    ...
                    generation_outcome_p14["result"] = result
                except Exception as exc:
                    generation_outcome_p14["error"] = str(exc)
        ...
        if not use_scenario_mode:
            ...
            if "error" in generation_outcome_p14:
                st.error(f"Test case generation failed: {generation_outcome_p14['error']}")
                st.stop()

            merged_result, group_results, duplicate_ids = generation_outcome_p14["result"]
            ...
            retried_groups = [r for r in group_results if r.get("retried")]
            ...
            if retried_groups:
                st.info(
                    f"{len(retried_groups)} group(s) automatically retried because the first "
                    f"attempt returned too few test cases: "
                    f"{', '.join(r['group_id'] for r in retried_groups)}."
                )
            ...
            if copied_example_dropped:
                st.warning(
                    ...
                    f"to the prompt's own worked example."
                )
        ...
        try:
            ...
            current_run = {
                ...
                "test_case_count": len(test_case_data["test_cases"]),
                "provider": provider,
                "model": generation_model,
            }

            st.session_state["phase14_results"] = current_run

            # Append to run history (keep last 5 runs)
            if "run_history" not in st.session_state:
                st.session_state["run_history"] = []

            st.session_state["run_history"].insert(0, current_run)
...
with tab_history:
    ...
    if not run_history:
        st.info("No runs yet this session. Generate test cases and they will appear here for download without needing to regenerate.")

    else:
        st.caption(
            "Last 5 runs this session. Download any previous run directly "
            "without clicking Generate again."
        )
        ...
        for i, run in enumerate(run_history):
            label = (
                ...
                f"{run.get('test_case_count', '?')} test cases  |  "
                f"{run.get('provider', '?')} / {run.get('model', '?')}  |  "
                f"{run.get('total_elapsed_time', '?')}s"
            )
            ...
            with st.expander(label, expanded=(i == 0)):
                ...
                with col1:
                    st.download_button(
                        label="Download Word",
                        data=run["word_data"],
                        file_name=f"test_cases_run{i+1}.docx",
                        mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                        key=f"history_word_{i}",
                    )
                ...
                with col2:
                    st.download_button(
                        ...
                        data=run["excel_data"],
                        file_name=f"test_cases_run{i+1}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        key=f"history_excel_{i}",
                    )

                with col3:
                    st.download_button(
                        label="Download Gap Report",
                        data=run["gap_report_markdown"],
                        file_name=f"coverage_gap_run{i+1}.md",
                        mime="text/markdown",
                        key=f"history_gap_{i}",
                    )

                summary_cols = st.columns(4)
                with summary_cols[0]:
                    st.metric("Test Cases", run.get("test_case_count", "?"))
                with summary_cols[1]:
                    st.metric("Requirement ID", run.get("active_requirement_id", "?"))
                with summary_cols[2]:
                    st.metric("Generation Time", f"{run.get('generation_elapsed_time', '?')}s")
                with summary_cols[3]:
                    st.metric("Provider", run.get("provider", "?"))
