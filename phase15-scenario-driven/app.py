...
st.title("Enterprise AI Test Case Generator - Phase 15")
st.caption(
    "Phase 15 introduces scenario-driven generation: Python extracts every "
    "testable scenario from your requirement, then the model writes one test "
    "case per scenario — guaranteeing maximum coverage on any requirement."
)
...
st.sidebar.caption(
    "Recommended: qwen2.5:7b for reliable multi-test-case generation. "
    "Requires ~8GB RAM. For real concurrency, run Ollama with "
...
generation_mode = st.sidebar.radio(
    "Generation Mode",
    [
        "Phase 15 - Scenario Driven (recommended, maximum coverage)",
        "Phase 14 - Coverage Group (legacy)",
    ],
    index=0,
)
use_scenario_mode = generation_mode.startswith("Phase 15")
...
st.sidebar.subheader("Phase 15 Settings")

max_parallel_workers = st.sidebar.slider(
    "Parallel scenario calls (match your OLLAMA_NUM_PARALLEL)",
    min_value=1, max_value=5, value=2,
    help="Set this to the same value as OLLAMA_NUM_PARALLEL. More = faster but needs more RAM."
)

max_scenarios = st.sidebar.slider(
    "Max scenarios to generate (0 = all)",
    min_value=0, max_value=50, value=0,
    help="Set to a lower number (e.g. 15) for a faster run with fewer test cases."
)
...
with tab_generate:
    ...
    if generate_clicked:
        ...
        if use_scenario_mode:
            ...
            st.subheader("Phase 15 - Scenario Extraction & Generation")
