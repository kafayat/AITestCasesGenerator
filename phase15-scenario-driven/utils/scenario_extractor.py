    ...
    for line in lines:
        if _contains_any(line, CONDITIONAL_KEYWORDS):
            ...
            if len(condition) > 5 and len(condition) < 300:
                desc = (
                    f"{field_name} is shown/hidden based on: {condition}"
                    if field_name
                    else condition
                )
...
def extract_scenarios(requirement_text):
    ...
    for desc, condition, field_name in conditional_rules:
        scenarios.append(_scenario(
            "conditional_rule",
            f"Conditional display rule: {field_name or desc[:60]}",
            context=desc,
            field_name=field_name,
        ))
    ...
    for criterion in criteria:
        # Skip criteria already covered by happy path / missing field
        if not any(
            _contains_any(criterion, [action.lower() for action in actions])
            and "successfully" in _lower(criterion)
            for _ in [1]
        ):
            scenarios.append(_scenario(
                "acceptance_criteria",
    ...
    for s in scenarios:
        ...
        if not any(_title_overlap(title_low, seen) > 0.80 for seen in seen_titles):
